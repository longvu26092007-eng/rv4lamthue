        ...
        local _okLoop, _errLoop = pcall(function()
            ...
            if isaccmain[myName] then
                if AB == "done" then
                    ...
                    -- getgenv().change=true → khi DONE thì ghi file "<tên>.txt" = "Completed-<tộc>" (ghi 1 lần).
                    -- change=false/nil → KHÔNG tạo. CHỈ tạo lúc done (set cờ SAU khi ghi thành công → fail thì thử lại).
                    if getgenv().change and not _G.changeFileWritten then
                        local _okw = pcall(function()
                            local race = game.Players.LocalPlayer.Data.Race.Value
                            writefile(game.Players.LocalPlayer.Name .. ".txt", "Completed-" .. tostring(race))
                        end)
                    ...
                    _G.changeFileWritten = false   -- hết done → reset cờ ghi file (done lại sẽ ghi lại)
                    if (myStatus == "in_trail" or myStatus == "moon") and not ab then
                        -- ĐANG FFA kill-player CỦA MÌNH (in_trail + ffup + ở temple) → GIỮ in_trail để kill
                        -- cho xong player; FFA hết (ffdown / rời temple) mới chuyển training. Tránh chuyển
                        -- training QUÁ SỚM → nhảy sang nhánh training → BỎ QUA khúc kill-player.
                        local inOwnFFA = (myStatus == "in_trail") and (templeState() == "ffup")
                            and (getdis(CFrame.new(28310.0234, 14895.1123, 109.456741)) < 2000)
