...
getgenv().fragmenttarget = getgenv().fragmenttarget or "........."
getgenv().id1 = getgenv().id1 or "........."
getgenv().id2 = getgenv().id2 or "........."
getgenv().id3 = getgenv().id3 or "........."
...
-- Kiểm tra đủ điều kiện chưa, nếu đủ thì gọi getgenv().client:ChangeToFolder(...)
-- theo đúng mẫu mà loader gốc đặt sẵn. Mỗi phiên chỉ chạy đúng 1 lần.
local function TryChangeFolder()
    if FragmentFolderLock then
        return false
    end
    if not getgenv().client then
        warn("[Fragment] getgenv().client chưa được set - bỏ qua đổi folder")
        return false
    end
    if typeof(getgenv().client.ChangeToFolder) ~= "function" then
        warn("[Fragment] getgenv().client.ChangeToFolder không tồn tại - bỏ qua đổi folder")
        return false
    end

    local rawTarget = getgenv().fragmenttarget
    if rawTarget == nil then
        warn("[Fragment] getgenv().fragmenttarget nil - bỏ qua đổi folder")
        return false
    end
    ...
    if not target or target <= 0 then
        warn(string.format(
            "[Fragment] getgenv().fragmenttarget không hợp lệ (%s) - bỏ qua đổi folder",
            tostring(rawTarget)
        ))
        return false
    end
    ...
    local id1 = getgenv().id1
    local id2 = getgenv().id2
    local id3 = getgenv().id3
    if id1 == nil or id2 == nil or id3 == nil
        or tostring(id1) == "" or tostring(id2) == "" or tostring(id3) == ""
        or tostring(id1):match("^%.+$") or tostring(id2):match("^%.+$") or tostring(id3):match("^%.+$")
    then
        warn("[Fragment] id1/id2/id3 rỗng - bỏ qua đổi folder")
        return false
    end
...
local function LoadAttack()
    ...
    getgenv().DragonTalonFastAttack = FastAttack
    getgenv().TyrantFastAttack = FastAttack
...
local function SetSkillAimTarget(target)
    getgenv().TyrantSkillAimTarget = GetAimPosition(target)
end

local function InstallSkillAimHook()
    if getgenv().TyrantSkillAimHookInstalled then
        return true
    end
